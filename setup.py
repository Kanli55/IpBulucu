#!usr/bin/env python3
#-*- coding:utf-8 -*-
#Başlangıç 22.11.2023 20:20
#Bitiş -:-
import os
os.system("clear")
os.system("termux-setup-storage")
os.system("apt install figlet")
os.system('clear')
os.system ("figlet SETUP FILE")
print("#Codded by Hacker KanlıSilah")
print("1 Yaz")
a = input(str("\nSeçim?:"))
if a == "1":
    print("8")
    os.system("sleep 1")
    print("7")
    os.system("sleep 1")
    print("6")
    os.system("sleep 1")
    print("5")
    os.system("sleep 1")
    print("4")
    os.system("sleep 1")
    print("3")
    os.system("sleep 1")
    print("2")
    os.system("sleep 1")
    print("1")
    os.system("sudo apt install git -y")
    os.system("sudo apt install python -y")
    os.system("sudo apt install python2 -y")
    os.system("sudo apt install python3 -y")
    os.system("sudo apt install bash -y")
    os.system("sudo apt install curl -y ")
    os.system("sudo apt install php -y")
    os.system("sudo apt install perl -y")
    os.system("sudo apt install busybox -y")
    os.system("sudo apt install cmatrix -y")
    os.system("sudo apt install dnsutils -y")
    os.system("sudo apt install hashdeep -y")
    os.system("sudo apt install hexcurse -y")
    os.system("sudo apt install json-c -y")
    os.system("sudo apt install openssl -y")
    os.system("sudo apt install sslscan -y")
    os.system("sudo apt install unzip -y")
    os.system("sudo apt install nmap -y")
    os.system("sudo apt install termux-tools -y")
    os.system("sudo apt install wget -y")
    os.system("sudo apt install root-repo")
    os.system("sudo apt install x11-repo")
    os.system("sudo apt install tur-repo")
    os.system("sudo apt update -y")
    os.system('pip install pyfiglet')
    os.system('pip install selenium')
    os.system('pip install requests')
    os.system('pip install pyfiglet')
    os.system('pip install sys')
    os.system('pip install py_compile')
    os.system('pip install youtube_dl')
    os.system('pip install uuid')
    os.system('pip install time')
    os.system('pip install os')
    os.system('pip install random')
    os.system('pip install datetime')
    os.system('pip install string')
    os.system('pip install secrets')
    os.system('pip install webbrowser')
    os.system('pip install hashlib')
    os.system('pip install colorama')
    os.system('pip install n')
    os.system('pip install mm')
    os.system('pip install new')
    os.system('pip install sleep')
    os.system('pip install BeautifulSoup')
    os.system('pip install new')
    os.system('pip install pafy')
    os.system('pip install bs4')
    os.system('pip install sys')
    os.system('pip install json')
    os.system('pip install random')
    os.system('pip install uuid')
    os.system('pip install secrets')
    os.system('pip install datetime')
    os.system('pip install argparse')
    os.system('pip install InstagramAPI')
    os.system('pip install sleep')
    os.system('pip install string')
    os.system('pip install uuid4')
    os.system('pip install generate_user_agent')
    os.system('pip install threading')
    os.system('pip install json')
    os.system('pip install datetime')
    os.system('pip install token_hex')
    os.system('pip install secrets')
    os.system('pip install token_hex')
    os.system('pip install Fore')
    os.system('pip install secrets')
    os.system('pip install uuid')
    os.system('pip install re')
    os.system('pip install b')
    os.system('pip install requests')
    os.system('pip install pyfiglet')
    os.system('pip install sys')
    os.system('pip install py_compile')
    os.system('pip install youtube_dl')
    os.system('pip install uuid')
    os.system('pip install time')
    os.system('pip install os')
    os.system('pip install random')
    os.system('pip install datetime')
    os.system('pip install string')
    os.system('pip install secrets')
    os.system('pip install webbrowser')
    os.system('pip install hashlib')
    os.system('pip install colorama')
    os.system('pip install n')
    os.system('pip install mm')
    os.system('pip install new')
    os.system('pip install sleep')
    os.system('pip install BeautifulSoup')
    os.system('pip install new')
    os.system('pip install pafy')
    os.system('pip install bs4')
    os.system('pip install sys')
    os.system('pip install json')
    os.system('pip install random')
    os.system('pip install uuid')
    os.system('pip install secrets')
    os.system('pip install datetime')
    os.system('pip install *')
    os.system('pip install argparse')
    os.system('pip install InstagramAPI')
    os.system('pip install sleep')
    os.system('pip install string')
    os.system('pip install uuid4')
    os.system('pip install generate_user_agent')
    os.system('pip install threading')
    os.system('pip install json')
    os.system('pip install datetime')
    os.system('pip install token_hex')
    os.system('pip install secrets')
    os.system('pip install token_hex')
    os.system('pip install Fore')
    os.system('pip install secrets')
    os.system('pip install uuid')
    os.system('pip install re')
    os.system('pip install b')
    os.system('pip install telethon')
    os.system('pip install pyshorteners')
    os.system('pip install finite_state_machine')
    os.system('cls' if os.name == 'nt' else 'clear') 
    os.system("apt upgrade -y")
    os.system("apt update -y && apt upgrade -y")
else:
    print("Yalnış seçim!!")
    os.system("sleep 3")
    os.system("python kanli.py")
